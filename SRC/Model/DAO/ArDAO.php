<?php
    ...
    function GetautotipusDAO(){
      ...
      $sql="SELECT `ID`,`Márka`,`Tipus`,(SELECT ID FROM `autokolcsonzo`.`Ar` WHERE `autokolcsonzo`.`autotipus`.`ID`= `autokolcsonzo`.`Ar`.`AutoTipusID` ) AS `ArID`,(SELECT Ár FROM `autokolcsonzo`.`Ar` WHERE `autokolcsonzo`.`autotipus`.`ID`= `autokolcsonzo`.`Ar`.`AutoTipusID` ) AS `Ar`,(SELECT Fajta_neve FROM `autokolcsonzo`.`fajta` WHERE `ID`=`Fajta_ID`) AS `Fajta` FROM `autokolcsonzo`.`autotipus`; ";
      $autoTipusokTomb=[];
